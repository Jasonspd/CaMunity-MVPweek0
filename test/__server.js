...
lab.experiment("When a user visits our homepage", function() {
	...
	lab.test("if not authenticated, homepage should exist", function(done) {
	...
	lab.test("if Jason is authenticated, it should redirect them to their profile page", function(done) {
		var options = {
			url: '/',
			method: 'GET',
			credentials: {
		...
		server.inject(options, function(response) {
			assert.equal(response.statusCode, 302, 'it should return a status code os 302');
			done();
		});
	});
...
// // testSTING THE STATUS-SO-FAR PAGE
